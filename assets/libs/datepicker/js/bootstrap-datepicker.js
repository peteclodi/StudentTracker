...
!function( $ ) {
	...
	Datepicker.prototype = {
		...
		fill: function() {
			...
			this.updateNavArrows();
			this.fillMonths();
            var beginningOfMonth = new Date(year, month);
			var prevMonth = new Date(beginningOfMonth.setDate(beginningOfMonth.getDay() === 0 ? 1 : 0));
			prevMonth.setDate(prevMonth.getDate() - (prevMonth.getDay() - this.weekStart));
			var nextMonth = new Date(year, month, DPGlobal.getDaysInMonth(year, month));
			nextMonth.setDate(nextMonth.getDate() + (this.weekEnd - nextMonth.getDay()));
